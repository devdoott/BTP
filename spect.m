function [Entropy,esmooth,stemp,stemps]=spect(x,y)
curdir = pwd;
%x='rec_1';
...
for i=1:820
    if(amp(i)~=0)
        Entropy=Entropy-(amp(i)*log(amp(i)));
    end
end
esmooth=0;
...
for i=1:820
    if(amp(i)~=0&&tot(i~=0))
        stemp=stemp+(amp(i)*log(amp(i)/tot(i)));
    end
end
%load('AATemplatewithSmoothing.mat');
amp=smooth(amp);

stemps=0;
for i=1:820
    if(amp(i)~=0&&tot(i~=0))
        stemps=stemps+(amp(i)*log(amp(i)/tot(i)));
    end
end

for i=1:820
    if(amp(i)~=0)
        esmooth=esmooth-(amp(i)*log(amp(i)));
    end
end
%folder='..\af\Atrial Analysis\';
%save(strcat(folder,x,'_AA_Entropy.mat'),'Entropy');
...
% xlabel('Frequency (Hz)','FontSize',14,'FontWeight','bold','Color','b')
%    ylabel('Normalized
%    Amplitude','FontSize',14,'FontWeight','bold','Color','b')%
   % title('Normalized frequecy spectrum','FontSize',16)
